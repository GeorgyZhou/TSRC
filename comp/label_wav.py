...
def run_graph(wav_data, labels, input_layer_name, output_layer_name,
              num_top_predictions, filename, res_dics):
...
def label_wav(wav_dir, labels, graph, input_name, output_name, how_many_labels):
  ...
  if not wav_dir or not tf.gfile.Exists(wav_dir):
    tf.logging.fatal('Audio file does not exist %s', wav_dir)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--wav_dir', type=str, default='', help='Audio files to be identified.')
  parser.add_argument(
  ...
  parser.add_argument(
      ...
      type=str,
      default='output.csv',
      help='Path to file to save test results'
  )
  parser.add_argument(
      '--output_name',
      type=str,
